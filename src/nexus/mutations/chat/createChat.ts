...
export const createChat = mutationField(t => t.nonNull.field('createChat', {
    ...
    resolve: async (_, { input }, ctx) => {
        ...
        // 해당 채팅방에 유저가 있는지 확인
        if (!chatRoomUsers.find(v => v.id === ctx.iUserId)) throw apolloError('채팅방에 속해 있지 않습니다', 'NO_PERMISSION')
        ...
        const chat = await ctx.prisma.chat.create({
            data: {
                ...
                notReadUserChatRoomInfos: { connect: userChatRoomInfos.filter(v => v.userId !== ctx.iUserId).map(v => ({ id: v.id })) }
            }
        })
        ...
        const notReadUserInfos = userChatRoomInfos
            .filter(v => v.userId !== ctx.iUserId) // 발신자 제외
            .filter(v => !!v.user.fcmToken) // fcmToken 없는 유저 제외

        for (const userInfo of notReadUserInfos) {
            try {
                console.log(userInfo.user.fcmToken)
                if (!userInfo.user.fcmToken) continue
                ...
                await userMessaging.send({
                    ...
                    data: {
                        ...
                        chatId: chat.id,
                        type: 'chat',
                        title: ctx.iUser.name,
                        message: chat.message || '사진',
                        subText: dayjs(chat.createdAt).format('a h:mm'),
                        image: ctx.iUser.image || USER_DEFAULT_PROFILE_IMAGE,
                    ...
                    android: {
                        notification: {
                            ...
                            priority: 'high',
                            imageUrl: ctx.iUser.image || USER_DEFAULT_PROFILE_IMAGE,
